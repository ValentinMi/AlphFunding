...
import { ALPH_TOKEN_ID, sleep, ZERO_ADDRESS } from "@alephium/web3";
...
describe("Pool", async () => {
  ...
  describe("Contribute", () => {
    ...
    it("should revert CollectIsFinished when contributing after the end", async () => {
      pool = (await deployPool({ ...defaultInitialFields, end: BigInt(Date.now() - 1000) })).contractInstance;
      ...
      const contributePromise = Contribute.execute(signer, {
        ...
        attoAlphAmount: alph(11) // +1 for map deposit
      });

      await expectAssertionError(contributePromise, pool.address, ErrorCodes.CollectIsFinished);
    });
    ...
    it("should revert NotEnoughFunds when contributing after the end", async () => {
      ...
      await expectAssertionError(contributePromise, pool.address, ErrorCodes.NotEnoughFunds);
    });
    ...
    it("should contribute to the pool and emit Contribute event", async () => {
      ...
      assert(totalCollected === alph(10))

      const { events } = await signer.nodeProvider.events.getEventsContractContractaddress(pool.address, { start: 0 })
      expect(events[0].eventIndex).toEqual(0)
      expect(events[0].fields).toEqual([{type: "Address", value: signer.address}, { type: 'U256', value: alph(10).toString() }, { type: 'U256', value: alph(10).toString() }])
    })
  })
  ...
  describe("Refund", () => {
    it("should revert CollectIsFinished when refunding after the end", async () => {
      ...
      await expectAssertionError(refundPromise, pool.address, ErrorCodes.CollectIsFinished);
    })

    it("should revert GoalAlreadyReached when refunding while pool goal is reached", async () => {
      pool = (await deployPool({ ...defaultInitialFields, totalCollected: alph(100) })).contractInstance;
      ...
      await expectAssertionError(refundPromise, pool.address, ErrorCodes.GoalAlreadyReached);
    })
    ...
    it("should revert NoContribution when refunding without previous contribution", async () => {
      ...
      await expectAssertionError(refundPromise, pool.address, ErrorCodes.NoContribution);
    })
    ...
    it("should refund the contribution", async () => {
      ...
      const balanceBeforeRefund = await balanceOf(ALPH_TOKEN_ID, signer.address)
  ...
  describe("Withdraw", () => {
    it("should revert NotTheBeneficiary if the caller is not the beneficiary", async () => {
     ...
     await expectAssertionError(withdrawPromise, pool.address, ErrorCodes.NotTheBeneficiary)
    })
    ...
    it("should revert CollectNotFinished when withdrawing before the end", async () => {
      ...
      await expectAssertionError(withdrawPromise, pool.address, ErrorCodes.CollectNotFinished);
    })
    ...
    it("should withdraw the funds to beneficiary if the caller is the creator", async () => {
      ...
      await Contribute.execute(signer, {
        ...
        attoAlphAmount: alph(101) // +1 for map deposit
      })
      ...
      assert(beneficiaryBalance === alph(100) + alph(1) / 10n) // add 0.1 ALPH for the minimal deposit transfer on destroy
    })

    it("should withdraw the funds to beneficiary if the caller is the beneficiary", async () => {
      const beneficiarySigner = new PrivateKeyWallet({ privateKey: testPrivateKey })

      const balanceOfBeneficiaryBefore = await balanceOf(ALPH_TOKEN_ID, beneficiarySigner.address)
      ...
      const balanceOfBeneficiaryAfter = await balanceOf(ALPH_TOKEN_ID, beneficiarySigner.address)
